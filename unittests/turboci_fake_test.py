...
def _mkOptions(id: str, *msg: type[Message]|Message, in_workplan: str = "") -> list[Check.OptionRef]:
...
def _mkResults(
    id: str, *msg: type[Message]|Message, in_workplan: str = "",
...
class IndexEntrySnapshotTest(test_env.RecipeEngineUnitTest):
  def test_snapshot(self):
    ...
    check = Check(
        ...
        results=[
          Check.Result(
              data=_mkResults('thing', Struct, Value),
          ),
        ],
        dependencies=deps,
    )
    ...
    self.assertEqual(entry.option_types, {
...
class SimpleTurboCIFakeTest(turboci_test_helper.TestBaseClass):
  def test_single_check_write(self):
    ...
    self.assertEqual(rslt.checks[0].option_data[0], _mkDatum(
  ...
  def test_check_state_PLANNING_add_dependency(self):
    ...
    self.assertEqual(rslt.check.dependencies[0], EdgeGroup(
        edges=[Edge(target=turboci.wrap_id(check_id('there')))]
    ))

  def test_error_check_state_missing_dep(self):
    with self.assertRaisesRegex(
        turboci.InvalidArgumentException,
  ...
  def test_check_PLANNING_to_FINAL(self):
    ...
    self.assertEqual(rslt.check.results[0].data[0].type_url, turboci.type_url_for(demoStruct))
  ...
  def test_query_filter_kind(self):
    self.write_nodes(
        turboci.check('a', kind='ANALYSIS'),
        turboci.check('b', kind='SOURCE'),
        turboci.check('c', kind='BUILD'),
        turboci.check('cc', kind='BUILD'),
    )

    ret = self.query_nodes(turboci.make_query(
        Query.Select.CheckPattern(kind=CheckKind.CHECK_KIND_ANALYSIS),
        Query.Select.CheckPattern(kind=CheckKind.CHECK_KIND_BUILD),
    ))
    self.assertEqual(len(ret.checks), 3)
    self.assertEqual(set(c.check.identifier.id for c in ret.checks), {'a', 'c', 'cc'})
  ...
  def test_query_filter_option(self):
    ...
    ret = self.query_nodes(turboci.make_query(
        Query.Select.CheckPattern(with_option_types=turboci.type_urls(demoStruct)),
    ))
  ...
  def test_query_filter_result(self):
    self.write_nodes(
        turboci.check('a', kind='ANALYSIS', results=[demoStruct], state='FINAL'),
        turboci.check('b', kind='ANALYSIS', results=[demoTS], state='FINAL'),
        turboci.check('c', kind='ANALYSIS', results=[demoStruct], state='FINAL'),
    )

    ret = self.query_nodes(turboci.make_query(
        Query.Select.CheckPattern(with_result_data_types=turboci.type_urls(demoStruct)),
    ))
  ...
  def test_query_filter_id_regex(self):
    self.write_nodes(
        turboci.check('a/b/c', kind='ANALYSIS'),
        turboci.check('butter', kind='SOURCE'),
        turboci.check('clorp', kind='BUILD'),
        turboci.check('neat', kind='BUILD'),
    )

    ret = self.query_nodes(turboci.make_query(
        Query.Select.CheckPattern(id_regex='.*[er].*'),
    ))
    self.assertEqual(len(ret.checks), 3)
    self.assertEqual(set(c.check.identifier.id for c in ret.checks), {
      'clorp', 'butter', 'neat'})
