...
from recipe_engine.fetch import CommitMetadata
...
class CommitList(object):
  ...
  def __init__(self, commit_list):
    ...
    # This maps dep_project_id -> dep_commit -> set(idxs)
    self._dep_idx = {}
    for i, c in enumerate(commit_list):
      self._rev_idx[c.revision] = i

      for dep_project_id, dep in c.spec.deps.iteritems():
  ...
  def from_repo_spec(cls, repo_spec):
    ...
    Args:
      repo_spec (package.RepoSpec) - The repo to get a CommitList from.

    Returns CommitList
    """
    return CommitList([repo_spec.current()] + repo_spec.updates())
  ...
  def dist_compatible_with(self, dep_project_id, dep_commit):
    """Returns the number of commits between the current CommitMetadata's commit
    and the next commit which would be compatible with the given
    (dep_project_id, dep_commit). This will essentially search through all the
    known future commits in this CommitList and find the closest one that
    depends on dep_project_id@dep_commit, and return the distance to that.

    If no commits from this repo depend on dep_project_id, this returns 0. If
    this repo depends on dep_project_id, but no compatibility with dep_commit is
    found, this returns len(#commits_to_HEAD).

    Args:
      dep_project_id (str) - The project_id that this repo possibly depends on.
    ...
    Returns int: The number of revisions between the current commit and the
      nearest future commit which depends on dep_project_id@dep_commit. May be 0
      if the current commit is compatible with the dependency already.
    """
    idx_table = self._dep_idx.get(dep_project_id)
    if not idx_table:
      return 0
