...
def RunSteps(api):
  ...
  try:
    api.file.copy('file not found - raise error',
...
def GenTests(api):
  yield api.test(
      'basic',
      api.step_data('file not found - raise error', api.file.errno('ENOENT')),
      api.step_data('file not found - fail silently', api.file.errno('ENOENT')),
      api.post_process(StepFailure, 'file not found - fail silently'),
