    ...
    if test.enabled:
      ...
      for dir_path, _, files in os.walk(self._backing_dir):
        for filename in files:
          ...
          all_paths.add(rel_path.replace('\\', '/'))
      def _read_fn(rel_path):
        with open(os.path.join(self._backing_dir, rel_path), 'rb') as fil:
          return fil.read()
...
class RawIOApi(recipe_api.RecipeApi):
  ...
  def output_dir(self, leak_to=None, name=None):
    ...
    Relative paths are stored '/'-delimited, and accessing paths using '\\' will
    convert them to '/' based paths.
    ...
    Example:
       ...
       assert result.raw_io.output_dir['some/file'] == 'contents of some/file'
       assert result.raw_io.output_dir['some\\file'] == 'contents of some/file'
       ...
       result.raw_io.output_dir['some/file'] -> raises KeyError
    """
